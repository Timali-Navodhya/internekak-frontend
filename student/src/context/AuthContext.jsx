...
export function AuthProvider({ children }) {
  ...
  const login = async (email, password) => {
    // Change the URL
    const response = await axios.post('http://localhost:5001/api/students/login', { email, password });
  ...
  const register = async (data) => {
    // Change the URL
    const response = await axios.post('http://localhost:5001/api/students/register', data);
    // Change 'admin' to 'student'
    const { token, student } = response.data.data;
    setAuthToken(token);
    setUser(student);
